...
rocket task ask <id> "<вопрос>" | --file <f> [--context <md>] [--to a,b]
    ...
    Отвечают той же ссылкой БЕЗ решётки: rocket task reply N/QM "..."
    (решётка в рамке читается как «задача», в аргумент её не переносят;
    для тредов ролей ссылка из рамки годится как есть: sre/Q1).
