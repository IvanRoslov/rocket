...
Совместимость провода: `asked_by` и `messages[].author` для человека по-прежнему приходят пустой строкой, тогда как `participants`, `waiting_on` и `addressed_to` содержат канонический `human`. Клиентам следует считать человеком оба варианта.
