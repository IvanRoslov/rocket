...
Поле `to` в запросах задаёт, **от кого ждут ответа** (`waiting_on`), и добавляет названных в участники; на доставку оно не влияет — каждая запись уходит всем участникам, кроме автора. У треда без сообщений очередь выводится из состава участников и автора вопроса, поэтому `to` при открытии треда добавляет и уведомляет адресатов, но очередь не сужает.
