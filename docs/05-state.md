...
Поэтому демон синхронизирует зеркала фоново, по тикеру (`mirror_sync_interval`, по умолчанию 5m):
...
**Плюс синхронно перед созданием worktree.** Фоновый тикер ходит раз в несколько минут, а worktree режется из зеркала — сессия, заспавненная в промежутке между тиками, получает точку ветвления, отстающую на десятки коммитов. Поэтому `spawn`, спавн оркестратора и `restore` подтягивают зеркало прямо перед `git worktree add`, той же операцией (`fetch --prune` + `merge --ff-only`) и с теми же тремя условиями — ничего не перетирается.

Этот синхронный синк ограничен таймаутом в 60s и **не блокирует спавн**: не уложился или упал — warn в лог, сессия создаётся на зеркале как есть. Протухшее зеркало — это худший worktree, а заблокированный спавн — это остановленная фича. Рабочие копии, зарегистрированные через `rocket repo add <path>`, не синкаются: rocket в них ничего не меняет.

### Блокировка зеркала

В зеркало пишут **четыре независимых процесса**: фоновый Syncer демона, `rocket repo sync`, `rocket repo sync --repair` и создание воркспейса (оно делает `git worktree add` внутри зеркала — это запись, а не чтение). Пока они не брали блокировку, они гонялись за git'овым `index.lock`: первый же прогон `--repair` при живом демоне оставил **18 зеркал с HEAD на `rescue/*`** (коммит прошёл, `checkout` гонку проиграл) и **26 залежавшихся `index.lock`** — ремонт оставил зеркала хуже, чем нашёл (#3589).

Поэтому каждый из четырёх берёт **эксклюзивный flock** на `<repos_dir>/.locks/<repo-id>.lock` и только потом трогает зеркало. В самом файле лежит запись о держателе (pid, операция, с какого момента), так что проигравший гонку говорит, *кто* держит зеркало, а не просто «занято». Ключевое: `--repair` держит лок на **всю** последовательность «rescue-коммит → checkout → fast-forward». Блокировка отдельных git-команд инцидент бы не предотвратила — зеркала застряли именно в промежутке между коммитом и checkout'ом.

Сколько каждый ждёт — в конфиге, и ждут они по-разному не случайно:

| Кто | Ключ | По умолчанию | Если не дождался |
|-----|------|--------------|------------------|
| фоновый Syncer | `mirror_lock_timeout_background` | 5s | пропускает зеркало до следующего тика (info в лог) |
| `repo sync`, `--repair` | `mirror_lock_timeout_cli` | 2m | печатает «занято другим процессом» с держателем, идёт к следующему зеркалу, код возврата 0 |
| создание воркспейса | `mirror_lock_timeout_clone` | 60s | **спавн падает** |

Фоновая метёлка ждёт мало намеренно: очередь за двухминутным `--repair` стоила бы тика всем остальным зеркалам, а метёлка придёт снова через пять минут. Спавн, наоборот, падает громко: клон зеркала посреди чужого merge отдаёт воркспейсу произвольный коммит, и ветка, срезанная не от той точки, — это неверный вывод, который никто не заметит часами. Обычная неудача синка (недоступный origin, заблокированное зеркало) спавн по-прежнему **не** валит — иначе каждый оффлайн-спавн падал бы.

**Уборка залежавшегося `index.lock`.** Держатель лока зеркала перед работой удаляет `index.lock` старше `mirror_index_lock_max_age` (по умолчанию 10m; `0s` выключает уборку совсем). Возраст — единственный критерий, и это безопасно ровно потому, что флок зеркала уже держим: ни один процесс rocket в этом зеркале сейчас быть не может. Принятая дыра одна — человек, запустивший долгий git руками прямо в зеркале; живые процессы мы не проверяем.

**Инвариант зеркала: оно всегда на своей default-ветке.** Рабочие ветки сессий живут только в `~/.rocket/worktrees/<repo-id>/<session-id>/`; сам rocket никогда не делает `checkout` рабочей ветки внутри зеркала — worktree режется командой `git worktree add`, которая HEAD зеркала не трогает. Поэтому любая посторонняя ветка, оказавшаяся в зеркале checked out, — след ручного вмешательства, и именно она блокирует fast-forward (`HEAD не на ветке <default>`).

**Спасательные ветки.** `rocket repo sync --repair` (см. [04-cli.md](04-cli.md)) чинит такое зеркало, ничего не теряя: незакоммиченные изменения — вместе с untracked-файлами — коммитятся в ветку `rescue/<YYYY-MM-DD-HHMMSS>`, после чего HEAD возвращается на default-ветку и выполняется обычный синк. Ранее вычекаученная ветка при этом не удаляется и не двигается — она остаётся ровно там, где была, и её всегда можно вычекаутить обратно вручную. Железное правило `workspace` действует и здесь: ветки мы не удаляем никогда.

**Чего `--repair` намеренно не делает.** Он не чинит разъехавшуюся default-ветку (`fast-forward невозможен`). Починка означала бы сдвинуть ветку с коммитов, которые она держит, — то есть потерять чужую работу. Такой случай остаётся человеку: rocket печатает причину и останавливается.

Свежесть зеркала (отставание в коммитах, время последнего fetch, признак `Blocked`) — производная величина: она вычисляется из самого git-репозитория, в rocket.db не хранится. Её печатают `rocket status`, `rocket repo ls` и `rocket task show` (см. [04-cli.md](04-cli.md)).
...
mirror_lock_timeout_clone: 60s      # сколько ждёт создание воркспейса, дальше спавн падает
mirror_index_lock_max_age: 10m      # с какого возраста залежавшийся .git/index.lock считается брошенным (0s — не убирать)
...
composer_busy_deadline: 10m # сколько доставку держат, пока в composer'е получателя лежит недописанный черновик человека
```
